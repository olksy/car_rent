...
export default function BodyType () {
    ...
    useEffect(() => {
        setLoading(true);
        axios
        .get(`http://127.0.0.1:8000/api/body-types/${bodyType}`)
        .then((response) => {
    ...
    return (
        <>
            <Helmet>
                <title>
                    {data.length > 0 &&
                        `${data[0].body_type.name} car Rental in Dubai, Hire ${data[0].body_type.name} at Rently`}
                </title>
            </Helmet>
            <Header />
            <div className="container pt-3 pt-lg-4 px-lg-0">
                <div className="d-block pb-sm-2">
                    <h1 className="type-h d-block d-lg-inline fs-36 mt-sm-3 mt-lg-0">
                        {data.length > 0 &&
                            `${data[0].body_type.name} cars for rent in Dubai`}
                    </h1>
                </div>
